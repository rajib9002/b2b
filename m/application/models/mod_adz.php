...
//        $this->db->update("bike_image", $imgdata, array('bike_id' => $bike_id));
        ...
        $this->session->unset_userdata('main_type_id');
        $this->session->unset_userdata('type_id_new');
...
//        $opt.='<option value="Other"> Other </option>';
    ...
    function get_model_options_model_selected($make_id = '', $sel = '') {
        $main_type_id = $this->session->userdata('parent_type_id');
        $type_id_new = $this->session->userdata('type_id_new');
        $query = $this->db->query("select * from model where main_type_id='$main_type_id' and type_id='$type_id_new' and  model_status=1  and make_id='$make_id'");
        $arr = $query->result_array();
        $opt.='<option value=""> --Select Model-- </option>';
        foreach ($arr as $val) {
            if ($val[model_name] == 'other' || $val[model_name] == 'Other') {
...
//    function update_my_bike_ad($bike_id = '') {
//
//        $bike = sql::row("bike", "bike_id=$bike_id");
//
//        $time = $bike['time'];
//
//
...
//    }
    ...
    function resize0($width, $height) {
        list($w, $h) = getimagesize($_FILES['file0']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file0']['name'];

        $file0 = $width . '_' . $height . '_' . $_FILES['file0']['name'];
        $this->session->set_userdata('file0', "$file0");

        $imgString = file_get_contents($_FILES['file0']['tmp_name']);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
        switch ($_FILES['file0']['type']) {
            case 'image/jpeg':
    ...
    function resize1($width, $height) {
        list($w, $h) = getimagesize($_FILES['file1']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file1']['name'];

        $file1 = $width . '_' . $height . '_' . $_FILES['file1']['name'];
    ...
    function resize2($width, $height) {
        list($w, $h) = getimagesize($_FILES['file2']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file2']['name'];

        $file2 = $width . '_' . $height . '_' . $_FILES['file2']['name'];
        $this->session->set_userdata('file2', "$file2");

        $imgString = file_get_contents($_FILES['file2']['tmp_name']);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
    ...
    function resize3($width, $height) {
        list($w, $h) = getimagesize($_FILES['file3']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file3']['name'];

        $file3 = $width . '_' . $height . '_' . $_FILES['file3']['name'];
        $this->session->set_userdata('file3', "$file3");

        $imgString = file_get_contents($_FILES['file3']['tmp_name']);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
    ...
    function resize4($width, $height) {
        list($w, $h) = getimagesize($_FILES['file4']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file4']['name'];

        $file4 = $width . '_' . $height . '_' . $_FILES['file4']['name'];
        $this->session->set_userdata('file4', "$file4");

        $imgString = file_get_contents($_FILES['file4']['tmp_name']);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
        switch ($_FILES['file4']['type']) {
            case 'image/jpeg':
                imagejpeg($tmp, $path, 100);
                break;
            case 'image/png':
    ...
    function resize($width, $height,$key) {
       // list($w, $h) = getimagesize($_FILES['images']['tmp_name'][$key]);
        ...
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['images']['name'][$key];

        $file0 = $width . '_' . $height . '_' . $_FILES['images']['name'][$key];
        $this->session->set_userdata('file0', "$file0");

        $imgString = file_get_contents($_FILES['images']['tmp_name'][$key]);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
    ...
    function resize5($width, $height) {
        list($w, $h) = getimagesize($_FILES['file5']['tmp_name']);
        $ratio = max($width / $w, $height / $h);
        $h = ceil($height / $ratio);
        $x = ($w - $width / $ratio) / 2;
        $w = ceil($width / $ratio);
        $path = '../uploads/bike_image/' . $width . '_' . $height . '_' . $_FILES['file5']['name'];

        $file5 = $width . '_' . $height . '_' . $_FILES['file5']['name'];
        $this->session->set_userdata('file5', "$file5");

        $imgString = file_get_contents($_FILES['file5']['tmp_name']);
        $image = imagecreatefromstring($imgString);
        $tmp = imagecreatetruecolor($width, $height);
        imagecopyresampled($tmp, $image, 0, 0, $x, 0, $width, $height, $w, $h);
        switch ($_FILES['file5']['type']) {
            case 'image/jpeg':
                imagejpeg($tmp, $path, 100);
                break;
            case 'image/png':
