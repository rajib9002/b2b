...
<h4 style="padding:10px;color:#000;background-color:#eee;margin:10px 0;">
My Saved Adz</h4>
    ...
    <?php
    $user_id = $this->session->userdata('user_id');
    $save_rows=sql::rows("save_ad","user_id='$user_id' and table_name!='' order by save_ad_id desc");
    
    $i = 0;
    if (count($save_rows) > 0):
        foreach ($save_rows as $s_row):
            //print_r($row);
            ...
            $tab_name = $s_row['table_name'] . '_image';
            $img = sql::row("$tab_name", "bike_id='$s_row[bike_id]' order by bike_image_id asc limit 0,1");
            
            
            $row= $this->mod_home->get_bike_details($s_row['table_name'], $s_row[bike_id]);
            
            ?>
           <div class="r_box_iphone float_left">

<a style="margin-top:3px;float: right;margin-right:6px;" onclick="return confirm('Are You sure to delete Saved adz?')" href="adz/delete_saved_adz/<?= $s_row['save_ad_id'] ?>/<?= $row['table_name'] ?>" title="Delete Saved Adz">
                            <img src="img/delete.jpg" style="height: 16px; width: 16px;"/>
                        </a>
                        ...
                        ?>
                        <div style="width: 128px;margin:6px auto 0px auto;">
                            <a href="<?php echo site_url('adz/details/' . $row[table_name] . '/' . $row['bike_id']); ?>"><img  src="<?php echo base_url() . '../uploads/bike_image/' . $bike_image; ?>" alt="<?= $row['model_name'] ?>" height="140" /></a>
                            <div class="clear"></div>
                            <span class="l_text" style="display: inline; font-weight: normal; font-size: 12px; color: #2462a3;width:43%;float:left;">
                                <?= $row['make_name'] ?>
                                <div class="clear"></div>
