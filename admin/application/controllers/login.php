...
class login extends Controller {
    ...
    function forgot_password() {
        ...
        if ($_POST['send']) {
            ...
            if ($this->form_validation->run()) {
                ...
                $this->session->set_flashdata('msg', 'Thank you for contacting GGAcademy Management Online Account Support.<p>We have Sent you an email to ' . $_POST['email'] . ' .
                                                    <span class="block">Please check your email- a password reset link will be included in a message from GGAcademy. Please click on the link to complete your password reset Process.</span></p>
                                                    <br />
                                                    <br />Thank you,
                                                    <br />GGAcademy Support');
    ...
    function is_user() {
        $valid = sql::count('users', "user_name='{$_POST['user_name']}' and user_email='{$_POST['email']}'");
        if ($valid > 0) {
            return TRUE;
        } else {
