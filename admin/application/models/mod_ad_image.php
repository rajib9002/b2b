...
class mod_ad_image extends Model {
    ...
    function get_typeGrid() {
      ...
      $con = " where 1";
        ...
        if ($_REQUEST['title'] != '')
            $con.=" and ad_image.title like '%$_REQUEST[title]%'";
        if ($status != '')
            $con.=" and ad_image.status like '%$status%'";

        $sql = "select * from ad_image  $con $sort";
        ...
        foreach ($rows as $row) {
            ...
            $image = "<img src='../uploads/ad_image/" . $row['image'] . "' alt='image' style='width:64px;height:64px;'/>";
    ...
    function add_image($file='', $pre_image='') {
        ...
        $param['dir'] = UPLOAD_PATH . "ad_image/";
        $param['type'] = "jpg,png,gif,jpeg";

        if (class_exists('zupload')) {
            $this->zupload->setUploadDir(UPLOAD_PATH . 'ad_image/');
        } else {
       ...
       function update_ad_image($id='',$image=''){
        $file = 'ad_image';
        if ($_FILES[$file]['name'] != '') {
            $image = $this->add_image($file);
        }

       $sql = "update ad_image set
