...
<!--<script type="text/javascript">
    ...
    function add_more_products(){ 
        var new_products='<tr>'+
            '<th>&nbsp;</th><td><select style="width:200px;" class="select-style2" name="competitionr_id[]"><?php echo $this->rider_model->get_competitionr_options()?></select><input type="hidden" name="pro_serial[]" value="" id="pr_serial_'+inc+'"/></td>'+
            '</tr>';
    ...
    <form id="valid_rider" action='<?= site_url($action); ?>' method='post' enctype="multipart/form-data">
        <table cellpadding=0 cellspacing=1>
            <tbody>               
				...
				<tr>    
            <td><th>&nbsp;</th><td>&nbsp;</td><td><a href="javascript:void(0)" style="color:#0072bc;font-weight: bold;" class="jmore_competition"><b>Add Multiple Competition</b></a></td>
                        </tr>
                    ...
                    foreach ($com as $c) { ?>
                        ...
                        <tr>
                            ...
                            <td><input type='hidden' name='pro_serial[]' value='' id='pr_serial_<?= $inc ?>'/>
                                <select style="width:200px;" class="select-style2" class="competition_<?= $inc ?>" name="competitionr_id[]"><?= common::get_competitionr_options($c[competitionr_id]) ?></select>
                            </td> 
...
<?php } else { ?>
                    ...
                    <tr>
                        ...
                        <td><input type='hidden' name='pro_serial[]' value='' id='pr_serial_<?= $inc ?>'/>
                            <select style="width:200px;" class="select-style2" class="competition_<?= $inc ?>" name="competitionr_id[]"><?= common::get_competitionr_options($competitionr_id) ?></select>
    <?= form_error('competitionr_id', '<span>', '</span>') ?>
                        </td> 
                        <td><a href="javascript:void(0)" style="color:#0072bc;font-weight: bold;" class="jmore_competition"><b>Add Multiple Competition</b></a>
...
<?= form_error('class_id', '<span>', '</span>') ?>
            ...
            <tr><th>&nbsp;</th>
                <td>
					...
					<input type="hidden" name="rider_number_s" value="<?= $rider_number ?>" />
					<input type="hidden" name="rider_first_s" value="<?= $rider_first_name ?>" />
					<input type="hidden" name="rider_last_s" value="<?= $rider_last_name ?>" />
					<input type="hidden" name="rider_country_s" value="<?= $country_id ?>" />
					<input type="hidden" name="rider_class_s" value="<?= $class_id ?>" />
