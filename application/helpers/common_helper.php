...
class common {
    ...
    function user_account_activation($email='', $user_name='') {
        ...
        if ($email != '') {
            ...
            $base_url = base_url();
            $msg_content = "Hi,<br>
            Thanks for Post Advertise in www.bike2biker.com<br>
            Thanks for your interest in www.bike2biker.com<br>
            ";

            $msg_content.= "<br /><br />Please click on the link below to Place a advertisement.
                        <br /><br />
                        Advertise link URL: <a href='" . site_url('adz/add_adz') . "'>Click Here For Advertisement</a>";
            ...
            $msg_content.= "<br /><br />Please click on the link below to Sign up with your email.
                        <br /><br />
                        Signup URL: <a href='" . site_url('home/signup') . "'>Click Here For New Account</a>";
    ...
    public static function get_bike_type($sel='') {
        $rows = sql::rows('type', 'type_status =1 and main_type_id<>0');
...
// $ts2='2013-06-20 09:59:18';
    ...
    function get_sql($condition = '') {
        $con = "select accessories.*,type.type_name, area.area_name, country.country_name,make.make_name,model.model_name
        from accessories
        left join type on type.type_id=accessories.type_id
        left join make on make.make_id=accessories.make_id
        left join model on model.model_id=accessories.model_id
        left join country on country.country_id=accessories.country_id
        left join area on area.area_id=accessories.area_id $condition
        UNION ALL
        select bycycles.*,type.type_name, area.area_name, country.country_name,make.make_name,model.model_name
        from bycycles
        left join type on type.type_id=bycycles.type_id
        left join make on make.make_id=bycycles.make_id
        left join model on model.model_id=bycycles.model_id
        left join country on country.country_id=bycycles.country_id
        left join area on area.area_id=bycycles.area_id $condition
        UNION ALL
        select motorparts.*,type.type_name, area.area_name, country.country_name,make.make_name,model.model_name
        from motorparts
        left join type on type.type_id=motorparts.type_id
        left join make on make.make_id=motorparts.make_id 
        left join model on model.model_id=motorparts.model_id
        left join country on country.country_id=motorparts.country_id
        left join area on area.area_id=motorparts.area_id $condition
        UNION ALL
        select offroad.*,type.type_name, area.area_name, country.country_name,make.make_name,model.model_name
        from offroad
        left join type on type.type_id=offroad.type_id
        left join make on make.make_id=offroad.make_id 
        left join model on model.model_id=offroad.model_id
        left join country on country.country_id=offroad.country_id
        left join area on area.area_id=offroad.area_id $condition
        UNION ALL
        select road_bikes.*,type.type_name, area.area_name, country.country_name,make.make_name,model.model_name
        from road_bikes
        left join type on type.type_id=road_bikes.type_id
        left join make on make.make_id=road_bikes.make_id 
        left join model on model.model_id=road_bikes.model_id
        left join country on country.country_id=road_bikes.country_id
        left join area on area.area_id=road_bikes.area_id $condition";
        return $con;
    }

    function get_sql1($condition = '') {
        $con = "select * from accessories $condition
        UNION ALL
        select * from bycycles $condition
        UNION ALL
        select * from motorparts $condition
        UNION ALL
        select * from offroad $condition
        UNION ALL 
        select * from road_bikes $condition";
        return $con;
    }
