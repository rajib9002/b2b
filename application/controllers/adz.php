...
function pay_now(){
            $transaction = $_POST['bk_id'];

if($transaction!=''){

            $url= "https://www.bkashcluster.com:9081/dreamwave/merchant/trxcheck/sendmsg?user=AABRACU&pass=A@8rC@247&msisdn=01622227228&trxid=$transaction";
            $ch = curl_init();
	    ...
	    $amount = $r['transaction']['amount'];
            ...
            $service = $r['transaction']['service'];
            $trxId = $r['transaction']['trxId'];
            $trxStatus = $r['transaction']['trxStatus'];
            $trxTimestamp = $r['transaction']['trxTimestamp'];
...
if($trxId==''){$trxId=0;}
if($trxStatus==''){$trxStatus=0;}
if($trxTimestamp==''){$trxTimestamp=0;}


header('Location: http://event.aabracu.net/login/pay_bkash/'.$amount.'/'.$counter.'/'.$currency.'/'.$receiver.'/'.$reference.'/'.$sender.'/'.$service.'/'.$trxId.'/'.$trxStatus.'/'.$trxTimestamp);
exit;
}else{
header('Location: http://event.aabracu.net/login/pay_bkash');
exit;
}
