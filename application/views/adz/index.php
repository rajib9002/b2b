...
<div class="search_result margin-bottom_10">
    ...
    <div class="p_box float_left">
        ...
        ?>
        <a href="<?php echo site_url('adz/details/'.$t_name.'/'. $row['bike_id']); ?>">
            <img src="<?php echo base_url() . 'uploads/bike_image/t_' . $bike_image; ?>" alt="<?= $row['model_name'] ?>" width="260" height="220" /></a>

        <div class="clear"></div>
        <span class="l_text" style="width:80px; display: inline"><?= substr($row['type_name'], 0, 10) ?></span>
        <span  <?php if ($row['ad_type'] == '1') { ?> class="round_shape" <? } if
                    ...
                    ($row['ad_type'] == '2') { ?> class="round_shape2" <? } ?> style="float: right; margin-top: -8px"><p><?= common :: get_ad_type_name($row['ad_type']) ?></p></span>
        <span  class="r_text">
                    <?php
